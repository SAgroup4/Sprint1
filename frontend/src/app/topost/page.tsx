...
// import styles from "./topost.module.css";
// import React, { useEffect, useState } from "react";
// import { useRouter } from "next/navigation";
...
export default function ToPostPage() {
  ...
  const [userId, setUserId] = useState("");
  const [userName, setUserName] = useState("匿名"); // 初始化 userName 為 "匿名"
  ...
  useEffect(() => {
    ...
    if (name) {
      setUserName(name); // 如果 localStorage 中有 userName，則更新 state
    }
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    const data = {
      ...
      content,
      user_id: userId,
      name: userName, // 使用 state 中的 userName
      skilltags,
      languagetags,
      leisuretags,
      trans: isTransfer,
